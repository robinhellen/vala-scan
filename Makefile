...
vala-scan.so: sane-backend-vapi/sane-backends.vapi $(LIBRARY_SOURCES)
	valac-0.26 --vapidir=sane-backend-vapi --pkg=sane-backends --pkg=gee-0.8 --pkg=gio-2.0 --library=vala-scan -H vala-scan.h $(LIBRARY_SOURCES) -X -fpic -X -shared -g -o vala-scan.so
